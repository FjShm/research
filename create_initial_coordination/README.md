...
|---|---|---|---|
|`mass_g_per_mole`|`dict`|{1: 60.8}|mass of each types of beads.<br>key: bead type<br>value: mass|
|`bond_length_Ang`|`dict`|{1: 5.}|bond length of each types of bonds.<br>key: bond type<br>value: length|
|`angle_degree`|`dict`|{1: 120.}|angle of each types of angles.<br>key: angle type<br>value: angle|
|`cell_length_Ang`|`float`|120.|Length of one side of the simulation box.|
...
|`overlap_threashold`|`float`|2.|Threshold at which particles are judged to overlap. If there are other particles within this distance of a particle when generating coordinates, the coordinate generation is redone.|
