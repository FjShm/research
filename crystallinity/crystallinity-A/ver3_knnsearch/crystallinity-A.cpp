...
int main(int argc, char* argv[]){
    ...
    const double k_nearest = param["k_nearest_neighbors"].as<double>();
    ...
    while(rd.read_1frame()){
        ...
        for (int i = 0; i < rd.num_atoms; i++){
            ...
            // kdtree.knnSearch()
            query = Point(position_fixed_coordinations[i]);
            std::vector<int> idxes = kdtree.knnSearch(query, k_nearest);
            for (int v : idxes) idxes_set.insert(v);
