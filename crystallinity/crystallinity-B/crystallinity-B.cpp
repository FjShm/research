...
int main(int argc, char* argv[]){
    ...
    for (int i = 0; i < max_loop; i++){
        ...
        // calculate C
        std::vector<double> C(beads_per_chain - 2*k - 1, 0.);
        for (int nc = 0; nc < num_chains; nc++){
            for (int alpha = k; alpha <= beads_per_chain - 2 - k; alpha++){
                ...
                sum /= sum.norm();
                double C_alpha = std::fabs(sum.dot(elongational_vector));
                C[alpha - k] += C_alpha;
            }
        }
        ...
        // average C
        for (int alpha = k; alpha <= beads_per_chain - 2 - k; alpha++){
            C[alpha - k] /= (double)num_chains;
        }

        // output crystallinity
        out_crytxt << timestep << " ";
        for (int alpha = k; alpha < beads_per_chain - 2 - k; alpha++){
            out_crytxt << C[alpha - k] << " ";
        }
        out_crytxt << C[beads_per_chain - 2 - k - k] << std::endl;
